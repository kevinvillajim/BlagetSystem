  ...
  const fetchProgress = async () => {
    try {
      ...
      progressData.forEach((progress) => {
        ...
        const courseUnits = progressData.filter(
          (p) => p.course_id === progress.course_id
        );
        const allUnitsCompleted = courseUnits.every((p) => p.completed);
        if (allUnitsCompleted) {
          localStorage.setItem(`Course${progress.course_id}isFinished`, "true");
        }
      });
    } catch (error) {
      console.error("Error fetching user:", error.response.data);
